...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const client = await connectToDatabase();
    try {
      const query = 'SELECT clerk_id FROM managers WHERE id = $1';
      const result = await executeWithRetry(() =>
        withTimeout(client.query(query, [id]), 10000)
      );
      ...
      await executeWithRetry(() =>
        withTimeout(client.query('DELETE FROM managers WHERE id = $1', [id]), 10000)
      );
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const client = await connectToDatabase();
    try {
      const clerkIdQuery = 'SELECT clerk_id FROM managers WHERE id = $1';
