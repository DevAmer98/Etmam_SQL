...
router.get('/supervisor', async (req, res) => {
  ...
  try {
   const limit = Math.min(parseInt(req.query.limit || '10', 10), 50);
      ...
      if (hasStatus) {
        params.push(status);
        whereParts.push(`(orders.status = $${idx} OR orders.supervisoraccept = $${idx})`);
        idx += 1;
      }
      if (username) {
        params.push(username);
        whereParts.push(`(LOWER(TRIM(orders.username)) = LOWER(TRIM($${idx})) OR LOWER(TRIM(clients.username)) = LOWER(TRIM($${idx})))`);
        idx += 1;
      }
