...
const sendWelcomeEmail = async (email, name, password, role) => {
  const content = `
    <div style="direction: rtl; text-align: right;">
      <h2>مرحبًا ${name}!</h2>
      <p>تم إنشاء حسابك كـ ${role === 'operation' ? ' مدير عمليات ' : role}.</p>
      <p>بيانات الدخول:</p>
      <p>الإيميل: ${email}</p>
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      const { name, email, phone, clerkId, role = 'operation', fcmToken = null } = req.body;
      ...
      const existing = await executeWithRetry(() =>
        withTimeout(client.query('SELECT * FROM operations WHERE email = $1', [email]), 10000)
      );

      if (existing.rows.length > 0)
        return res.status(400).json({ success: false, message: 'Operation already exists' });
      ...
      const insert = `
        INSERT INTO operations (name, email, phone, clerk_id, role, created_at, fcm_token)
